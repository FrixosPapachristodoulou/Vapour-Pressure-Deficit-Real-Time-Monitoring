# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 12:26:34 2024
...
def plot_colored_lines(ax, x_values, y_values, threshold=706):
    for i in range(len(y_values) - 1):
        ...
        if y[0] <= threshold and y[1] <= threshold:
            ...
            # If the line crosses the threshold, split it
            slope = (y[1] - y[0]) / (x[1] - x[0])
            crossing_x = (threshold - y[0]) / slope + x[0]

            # Plot the segment below the threshold
            if y[0] < threshold:
                ax.plot([x[0], crossing_x], [y[0], threshold], color='green')
                ax.plot([crossing_x, x[1]], [threshold, y[1]], color='red')
            else:
                ax.plot([x[0], crossing_x], [y[0], threshold], color='red')
                ax.plot([crossing_x, x[1]], [threshold, y[1]], color='green')
...
def main():
    ...
    while True:
        with live_update.container():
            ...
            st.markdown("<div class='section-title'>Latest 10 Days</div>", unsafe_allow_html=True)
